...
function createJob(index) {
  ...
  container.innerHTML = `
    <label class="entry-title">Job Title</label>
    <input type="text" class="jobtitle styled" placeholder="Software Engineer">
    <button class="remove-job button" id="delete-button" type="button"><img src="/assets/images/trash.png" id="garbagePNG"></button>
    <br>
...
function switchToSkill(skillData, container){
    ...
    editor.innerHTML = `
        <input class="skill-name styled" id="skillName-input" type="text" placeholder="Skill name" value="${skill.name}">
        <textarea class="skill-description" placeholder="Describe how you used this skill" >${skill.description}</textarea>
        <button class="remove-skill button" id = "skill-button" type="button">Remove Skill</button>
    `;
